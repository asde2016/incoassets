...
<template>
  <DialogPortal>
    ...
    <DialogContent
      ...
      :class="
        cn(
          'fixed left-[50%] top-[50%] z-50 grid w-full max-h-[calc(100dvh-32px)] grid-rows-[auto_minmax(0,1fr)_auto] translate-x-[-50%] translate-y-[-50%] gap-24 rounded-lg bg-background p-32 shadow-lg',
      ...
      ">
      <slot name="header" />
      <div class="min-h-0 overflow-y-auto -mx-32 px-32">
        <BodySlot />
      </div>
      <slot name="footer" />
      <DialogClose
